...
Description: Style post title with several strategies (Strategypattern).
Version: 0.0.1
*/

namespace WP\Post_Title_Styler;

// Hack to get the corret path since we did symlink plugins via provisioner.
define('WP_PTS_PATH', sprintf('%s/%s/', WP_PLUGIN_DIR, basename(dirname(__FILE__))));

define('WP_PTS_DOMAIN', 'WP_PTS');
...
class Strategy_Context
{
    ...
    {
        $classname = "WP\\Post_Title_Styler\\{$strategy}";
...
class Uppercase_First_Strategy implements Strategy
{
    ...
    public static function get_label()
    {
        return __('Uppercase first char', WP_PTS_DOMAIN);
    }
}
...
class Uppercase_All_Strategy implements Strategy
{
    ...
    public static function get_label()
    {
        return __('Uppercase all chars', WP_PTS_DOMAIN);
    }
}
...
class Lowercase_All_Strategy implements Strategy
{
    ...
    public static function get_label()
    {
        return __('Lowercase all chars', WP_PTS_DOMAIN);
    }
}
...
class WP_Plugin
{
    ...
    const STYLER_POSTMETA = 'WP_PTS';
    const FIELD_ID = 'WP_pts_strategy';

    /**
     * List of classes which implement `WP_PTS_Strategy`.
     *
     * @var array
     */
    private $strategies;
    ...
    public function on_save_post($post_id)
    {
        ...
        $in_expected_condition = (
            ...
            &&
            wp_verify_nonce($_POST[__CLASS__], WP_PTS_PATH . basename(__FILE__))
        );
    ...
    public function on_add_meta_boxes()
    {
        extract(array(
            ...
            'title' => __('Post title styler', WP_PTS_DOMAIN),
    ...
    {
        wp_nonce_field(WP_PTS_PATH . basename(__FILE__), __CLASS__);
        ...
        <p class="howto">
            <?php _e('Choose the strategy on how the title will be styled.', WP_PTS_DOMAIN); ?>
        </p>
        <?php
    }

    /**
     * Callback for `the_title` filter. This method asks WP_PTS_Strategy_Context
    ...
    /**
     * Collect all classes which implement `WP_PTS_Strategy` interface.
    ...
    private function _get_strategies()
    {
        if (!$this->strategies) {
            ...
            foreach (get_declared_classes() as $class) {
                ...
                if ($reflection->implementsInterface('WP\Post_Title_Styler\Strategy'))
